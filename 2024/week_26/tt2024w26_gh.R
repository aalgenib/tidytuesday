...
movie_votes <- lgbtq_movies |> 
  ...
  mutate(original_language = factor(original_language, levels = c("English", "French", "Spanish", "Thai", "Korean")))


# set text content -------------------------------------------------------------

txt_title <- "Among the vast amount of English movies relevant to<br>the LGBTQ+ community a Korean film catches the eye"
txt_subtitle <- "Average user score of movies in *The Movie Database* (TMDB) with assigned LGBTQ+ relevant<br>keywords<sup>#</sup>, from 1975 to 2021. A score of 10 would be highest. Notice that this chart only<br>includes movies which have been rated by a minimum of 1,000 user votes in order to have a<br>solid sample size for scoring."
...
grob_korean <- grobTree(
  textGrob("The Handmaiden", x = 0.75, y = 0.95, hjust = 1, vjust = 1, gp = gpar(family = ft1, fontface = "bold", fontsize = 13, col = "#EF476F")),
  textGrob("avg. score: 8.3\nvotes: 2,682", x = 0.75, y = 0.90, hjust = 1, vjust = 1, gp = gpar(family = ft1, fontface = "plain", fontsize = 13, col = "#EF476F"))
)
...
ggplot(movie_votes, aes(release_date, vote_average, colour = original_language, size = vote_count)) +
  ...
  theme(
    ...
    plot.caption = element_markdown(hjust = 0, size = 10, family = ft1, margin = margin(t = 0.8, unit = "cm"), colour = col_text_x_axis, lineheight = 1.2),
  ...
  # korean film
  annotate(geom = "segment", x = as.Date("2012-04-01"), y = 8.3, xend = as.Date("2015-06-01"), yend = 8.3, colour = "#EF476F") +
